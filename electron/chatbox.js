...
class ChatBox {
  constructor({ inputEl, sendBtn, answerArea, fileBtn, fileInput, badge }) {
    ...
    if (this.transcript && "readOnly" in this.transcript) {
    ...
    // Defensive: make transcript read-only (legacy code path kept for reference)
    if (this.transcript && "readOnly" in this.transcript) {
      this.transcript.readOnly = true;
    }
    if (this.transcript) {
  ...
  _isStatusyBanner(t) {
    ...
    return (
      ...
      /\b(PDF support not installed|PDF load error|Web\+\s+(enabled|disabled))\b/i.test(
        x
      ) ||
      /^\s*Tip:\s+/i.test(x) ||
      /^\s*Status:\s+/i.test(x)
  ...
  async submit() {
    ...
    // Mirror typed text into the single Transcript sink
    this._appendToTranscript(`You: ${q}`);

    // Clear input
    this.input.value = "";

    // Ask backend (no AI bubble to transcript)
    ...
    // Answers go ONLY to the Answer area (textarea or div)
  ...
  async ingest() {
    ...
    try {
      if (f.name.toLowerCase().endsWith(".txt")) {
        ...
        const res = await window.electron.invoke("doc:ingestText", {
          name: f.name,
          text
        });
        ...
        const res = await window.electron.invoke("doc:ingestBinary", {
          name: f.name,
          bytes,
